...
class CNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1), # input : 1, output(filter 갯수) : 32, kernel_size : 3x3
            torch.nn.ReLU(),
    ...
    def forward(self, x):
        out = self.layer1(x)
        out = self.layer2(out)
        out = out.view(out.size(0), -1)   # Flatten them for FC # .view(shape 지정)
        out = self.fc(out)
        return out
...
for epoch in range(training_epochs):
    avg_cost = 0
    total_batch = len(mnist_train) // batch_size # // → 나누고 몫만

    for i, (batch_xs, batch_ys) in enumerate(data_loader): # enumerate 순차적으로 목록화
        ...
        cost.backward() # cost 함수에 대해 back propagation(미분)
        optimizer.step()
